...
<form action="<%= request.getContextPath() %>/board/boardCommentDelete" name="boardCommentDelFrm" method="POST">
	<input type="hidden" name="commentNo" /><!-- 댓글 번호 -->
	<input type="hidden" name="boardNo" value="<%= board.getNo() %>"/>
</form>
...
document.querySelectorAll(".btn-reply").forEach((button) => {
	button.onclick = (e) => {
		...
		// console.log(parent); // td - tr - tboby
		
		// 2. 기준요소찾기 다음tr태그 : 현재타켓 버튼리플라이 
		const ref = e.target.parentElement.parentElement.nextElementSibling;
		// console.log(ref);
		
		// 생성된 tr 추가 : beforebegin 동일하게작동
		// parent.insertBefore(tr, ref); // 부모요소하고 기준이되는 요소 두개를 알아야함.

		// target.insertAdjacentElement(position, newNode)
		// 1. beforebegin : target의 이전 형제요소로 추가
		// 2. afterbegin : target의 시작태그 다음에 자식요소로 추가 
		// 3. beforeend : target의 종료태그 직전에 자식요소로 추가 (append와동일)
...
const commentSubmitHandler = (e) => {
	...
	// 정규식을 만족하지 못할 때 (아무글자 혹은 개행문자포함 한글자이상인지)
	if(!/^(.|\n)+$/.test(contentVal)){
		alert("댓글 내용을 작성해주세요.");
